...
template <class Messenger>
   double CalculateSoftSum(FourVector &J, Messenger &M, double R, double PTMax, double Power);
...
int main(int argc, char *argv[])
{
   ...
   double JetSF00[MAX], JetSF10[MAX], JetSF15[MAX], JetSF20[MAX], JetSF25[MAX], JetSF30[MAX];
   ...
   Tree.Branch("JetSF00", &JetSF00, "JetSF00[NJet]/D");
   Tree.Branch("JetSF10", &JetSF10, "JetSF10[NJet]/D");
   Tree.Branch("JetSF15", &JetSF15, "JetSF15[NJet]/D");
   Tree.Branch("JetSF20", &JetSF20, "JetSF20[NJet]/D");
   Tree.Branch("JetSF25", &JetSF25, "JetSF25[NJet]/D");
   Tree.Branch("JetSF30", &JetSF30, "JetSF30[NJet]/D");
   ...
   for(string FileName : InputFileName)
   {
      ...
      for(int iE = 0; iE < EntryCount; iE++)
      {
         ...
         for(int iJ = 0; iJ < (int)Jets.size(); iJ++)
         {
            ...
            JetSF00[NJet] = CalculateSoftSum(Jets[iJ], MPF, 0.4, 0.0, 1);
            JetSF10[NJet] = CalculateSoftSum(Jets[iJ], MPF, 0.4, 1.0, 1);
            JetSF15[NJet] = CalculateSoftSum(Jets[iJ], MPF, 0.4, 1.5, 1);
            JetSF20[NJet] = CalculateSoftSum(Jets[iJ], MPF, 0.4, 2.0, 1);
            JetSF25[NJet] = CalculateSoftSum(Jets[iJ], MPF, 0.4, 2.5, 1);
            JetSF30[NJet] = CalculateSoftSum(Jets[iJ], MPF, 0.4, 3.0, 1);
   ...
   double CalculateSoftSum(FourVector &J, Messenger &M, double R, double PTMax, double Power)
{
   ...
   {
      if(M.Tree == nullptr)   return 0;
      if(M.PT == nullptr)     return 0;
      if(M.Eta == nullptr)    return 0;
      if(M.Phi == nullptr)    return 0;
      ...
      for(int i = 0; i < N; i++)
      {
         ...
         double DEta = M.Eta->at(i) - J.GetEta();
         double DPhi = M.Phi->at(i) - J.GetPhi();
         if(DPhi < -M_PI)   DPhi = DPhi + 2 * M_PI;
         if(DPhi > +M_PI)   DPhi = DPhi - 2 * M_PI;
         if(DEta * DEta + DPhi * DPhi > R * R)
            continue;

         if(Power == 1)
         {
            FourVector P;
            P.SetPtEtaPhiMass(M.PT->at(i), M.Eta->at(i), M.Phi->at(i), 0);
            VSum = VSum + P;
         }
         else if(Power == 0)
            Sum = Sum + 1;
         else
            Sum = Sum + pow(M.PT->at(i), Power);
      }
   }
