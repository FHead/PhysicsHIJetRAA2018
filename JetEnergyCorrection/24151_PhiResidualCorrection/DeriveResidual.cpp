   ...
   for(string FileName : FileNames)
   {
      ...
      for(int iE = 0; iE < EntryCount; iE++)
      {
         ...
         for(int iJ = 0; iJ < NJet; iJ++)
         {
            ...
            int IEta = -1;
            if(BaseOnGen == true)
               IEta = FindBin(GenJetEta->at(iJ), EtaBinning);
            else
               IEta = FindBin(MatchedJetEta->at(iJ), EtaBinning);
            ...
            if(BaseOnGen == true)
               Jets[IEta].push_back({GenJetPT->at(iJ), GenJetEta->at(iJ), GenJetPhi->at(iJ), AverageRho, R});
            else
               Jets[IEta].push_back({MatchedJetPT->at(iJ), MatchedJetEta->at(iJ), MatchedJetPhi->at(iJ), AverageRho, R});

            if(BaseOnGen == true)
               POverall.Fill(GenJetPhi->at(iJ), R);
            else
               POverall.Fill(MatchedJetPhi->at(iJ), R);
         }
      }
         ...
         for(int iPT = 0; iPT < NPT; iPT++)
         {
            ...
            for(Jet J : RhoJets)
               if(J.PT >= PTBinning[iRho][iPT] && J.PT < PTBinning[iRho][iPT+1])
                  P[iEta][iPT][iRho]->Fill(J.Phi, J.R);
