   ...
   for(int iEta = 0; iEta < NEta; iEta++)
   {
      ...
      for(int iRho = 0; iRho < NRho; iRho++)
      {
         ...
         for(int iPT = 0; iPT < NPT; iPT++)
         {
            ...
            DHFile[DHState][KeyBase+"_GenPTMean"] = TotalPT / JetCount;
            DHFile[DHState][KeyBase+"_JetCount"] = JetCount;
      ...
      for(int iRho = 0; iRho < NRho; iRho++)
      {
         vector<vector<pair<double, double>>> Cs(NPT);
         for(int iPT = 0; iPT < NPT; iPT++)
            Cs[iPT] = CalculateCn(P[iEta][iPT][iRho], NMax);
         vector<double> AdjustedBinning = PTBinning[iRho];
         for(int iPT = 0; iPT < NPT - 1; iPT++)
            AdjustedBinning[iPT+1] = PTBinning[iRho][iPT+1] * (Cs[iPT][0].first + Cs[iPT+1][0].first) / 2;
         ...
         for(int iPT = 0; iPT < NPT; iPT++)
         {
            ...
            H0.Fill(PTCenter, RhoCenter, Cs[iPT][0].first);
            H1C.Fill(PTCenter, RhoCenter, NMax >= 1 ? Cs[iPT][1].first : 0);
            H1S.Fill(PTCenter, RhoCenter, NMax >= 1 ? Cs[iPT][2].first : 0);
            H2C.Fill(PTCenter, RhoCenter, NMax >= 2 ? Cs[iPT][3].first : 0);
            H2S.Fill(PTCenter, RhoCenter, NMax >= 2 ? Cs[iPT][4].first : 0);
            H3C.Fill(PTCenter, RhoCenter, NMax >= 3 ? Cs[iPT][5].first : 0);
            H3S.Fill(PTCenter, RhoCenter, NMax >= 3 ? Cs[iPT][6].first : 0);
            H4C.Fill(PTCenter, RhoCenter, NMax >= 4 ? Cs[iPT][7].first : 0);
            H4S.Fill(PTCenter, RhoCenter, NMax >= 4 ? Cs[iPT][8].first : 0);
            ...
            DHFile[DHState][KeyBase+"_PTMean"]     = DHFile[DHState][KeyBase+"_GenPTMean"].GetDouble() * Cs[iPT][0].first;
            ...
            for(int i = 0; i < (int)Cs[iPT].size(); i++)
            {
               DHFile[DHState][KeyBase+"_P"+to_string(i)] = Cs[iPT][i].first;
               DHFile[DHState][KeyBase+"_E"+to_string(i)] = Cs[iPT][i].second;
            }
         }
      }
